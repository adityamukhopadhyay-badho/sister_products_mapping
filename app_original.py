#!/usr/bin/env python3
"""
FastAPI Application for Sister Products Mapping System

This application provides a REST API for processing sister products mapping
with the ability to process specific brands and download results as CSV.
            ...
            document.getElementById('processingForm').addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        status.innerHTML = '✅ Sister products processing completed! CSV file downloaded.';
                        
                        results.style.display = 'block';
                        results.innerHTML = `
                            <h3>Processing Complete</h3>
                            <p>Your sister products mapping has been completed and the CSV file has been downloaded.</p>
...
async def process_brand_sister_products(request: BrandProcessingRequest):
    ...
    This endpoint:
    1. Fetches all products for the given brandId from database
    2. Processes them through the sister products mapping pipeline
    3. Returns the results as a downloadable CSV file
    """
    try:
        ...
        processing_status[request.brandId].total_clusters = results['total_clusters']
        processing_status[request.brandId].message = f"Processing complete! Found {results['total_clusters']} sister product clusters."
...
def create_sister_products_csv(processed_df: pd.DataFrame, results: Dict[str, Any]) -> pd.DataFrame:
    ...
    Args:
        processed_df: Processed DataFrame with clustering results
        results: Results dictionary from sister products mapping
        
    Returns:
        DataFrame in the format: brandId, clusterId, brandSKUId, sisterProductId
    """
